            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="jarvis-card p-4">
                  ...
                  <div className="h-64">{isMounted && statusChart.length > 0 ? (<ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={statusChart} cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value" isAnimationActive={false}>{statusChart.map((_, i) => <Cell key={i} fill={['#00f0ff', '#3b82f6', '#8b5cf6', '#10b981', '#f59e0b', '#ef4444', '#6b7280'][i % 7]} />)}</Pie><Tooltip content={<CustomTooltip />} /><Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: '11px', fontFamily: 'monospace' }} /></PieChart></ResponsiveContainer>) : (<div className="flex items-center justify-center h-full"><p className="text-xs text-gray-500 font-mono">Nenhum dado disponível</p></div>)}</div>
                </div>
              </div>
